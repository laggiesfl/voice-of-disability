...
export async function guardPublicForm(
  ...
  const allowedOrigins = new Set([
    'https://www.voiceofdisability.com',
    'https://voiceofdisability.com',
  ]);

  if (process.env.NODE_ENV === 'production' && origin && !allowedOrigins.has(origin)) {
    return { ok: false, status: 403, error: 'This submission could not be verified.' };
  }

  if ((payload.website ?? '').trim() !== '') {
    return { ok: false, status: 400, error: 'This submission could not be verified.' };
  }

  const startedAt = Number(payload.startedAt);
  const elapsed = Date.now() - startedAt;
  if (!Number.isFinite(startedAt) || elapsed < MIN_FILL_MS || elapsed > MAX_FILL_MS) {
    return { ok: false, status: 400, error: 'Please reload the form and try again.' };
  }
  ...
  if (!secret) {
    return {
      ok: false,
      status: 503,
      error: 'Online forms are temporarily unavailable. Please email fadila@voiceofdisability.com.',
    };
  }

  const token = (payload.turnstileToken ?? '').trim();
  if (!token) {
    return { ok: false, status: 400, error: 'Please complete the security check and try again.' };
  }
  ...
  try {
    ...
    if (!result.success) {
      return { ok: false, status: 400, error: 'The security check could not be verified. Please try again.' };
    }
  } catch {
    return { ok: false, status: 503, error: 'The security check is temporarily unavailable. Please try again later.' };
  }
